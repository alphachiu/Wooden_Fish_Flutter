...
import 'package:woodenfish_bloc/ui/custom_bg/models/custom_object_models.dart';

class FallingBirdsPainter extends CustomPainter {

  FallingBirdsPainter({required this.objectImage, required this.notifier}) : super(repaint: notifier);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (objectImage != null) {
      ...
      if (random.nextDouble() < 0.30) {
        // drop new bird
        birds.add(CustomObjectModel(ms, spriteRects[random.nextInt(10)], List.generate(10, (i) => random.nextDouble()), size));
      }

      final transforms = birds.map((bird) => bird.transform(ms, size)).toList();
      final rects = birds.map((bird) => bird.rect).toList();

      canvas.drawAtlas(objectImage!, transforms, rects, null, null, null, imagePaint);
